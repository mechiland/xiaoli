// Generates server/extract/prompts.generated.ts from prompts/*.md so the Worker bundle needs no fs access.
// Run: node --import tsx server/extract/scripts/gen-prompts.ts   (prompts.test.ts fails when the file is stale)
import { readdirSync, readFileSync, writeFileSync } from 'node:fs'
import path from 'node:path'
import { fileURLToPath } from 'node:url'
import { parsePromptFile, type PromptFile } from '../prompt-file'

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../..')
export const PROMPTS_DIR = path.join(ROOT, 'prompts')
export const GENERATED_PATH = path.join(ROOT, 'server/extract/prompts.generated.ts')
...
export function renderGenerated(files: PromptFile[]): string {
  ...
  return `// GENERATED from prompts/*.md by server/extract/scripts/gen-prompts.ts. Do not edit by hand.
