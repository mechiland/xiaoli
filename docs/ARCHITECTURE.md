...
ExtractionOutputSchema = z.object({
  ...
  loops:      z.array(z.object({ person: PersonRef, direction: z.enum(['mine','theirs','mutual']), kind: z.enum(['promise','question','plan']), text: z.string().min(2).max(200), dueAt: z.string().max(20).optional(), evidence: Evidence }).strict()).default([]),
...
- `loops`: person_id fk persons cascade, direction text ('mine'|'theirs'|'mutual'), kind text ('promise'|'question'|'plan'), text text, text_norm text, due_at text null (PartialDate), opened_message_id fk messages **(set null)**, opened_at text (MsgTime), closed_message_id int null fk messages (set null), closed_at text null (MsgTime), closed_reason text null ('done'|'dropped'), status (proposed|confirmed|rejected), import_id null fk imports (set null), job_id null, source_kind. idx (owner_id, person_id, status), idx (owner_id, import_id), idx (owner_id, status, text_norm), idx (owner_id, due_at)
...
export interface WindowInput {
  ...
  known: { personId: number; label: string; handles: { kind: HandleKind; value: string }[]; claims: { id: number; statement: string; category: Category }[]
           ...
           openLoops?: { id: number; kind: 'promise'|'question'|'plan'; direction: 'mine'|'theirs'|'mutual'; text: string; openedAt: MsgTime }[] }[]
}
// validation — pure
...
export interface OfflineExtractionResult {
  ...
  loops:     { person: string; direction: 'mine'|'theirs'|'mutual'; kind: 'promise'|'question'|'plan'; text: string; dueAt?: string; openedAt: MsgTime; openedIdx: number; closedIdx: number | null; closedAt: MsgTime | null; closedReason: 'done'|'dropped' | null; evidence: number[]; windowIndex: number }[]
...
GoldFile {
  ...
  loops?:    { id: string, person: string, direction: 'mine'|'theirs'|'mutual', kind: 'promise'|'question'|'plan', text: string, dueAt?: string, evidence: number[] /*the message that opens it*/, closedBy?: number /*idx of the message that closes it, if any*/, closedReason?: 'done'|'dropped', optional?: boolean }[],
...
| home (3) | `@/server/interaction.getUpcomingPlans`, `@/components/interaction.PlanRow` | interaction (5) — mounted by the integrator in wave 5 |
