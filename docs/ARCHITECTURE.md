...
- **Layout**: application code (what ships in the Worker/browser bundle) lives under `src/` — `src/app`, `src/components`, `src/contracts`, `src/lib`, `src/server`, `src/styles`. Tooling stays at the repo root: `scripts/` (CLI entries; a module's own CLIs live in `scripts/<module>/`), `eval/`, `verify/`, `tests/` (shared helpers, e2e, integration), `fixtures/`, `prompts/` (`prompts/judge/` = eval judge prompts), `drizzle/`, `docs/` (this file, `SPEC.md`, `PLAN.md`, `DECISIONS.md`, …). Application code never imports from the tooling directories (tests excepted).
- Import aliases: `@/` = `src/`, `~/` = repo root (tooling: `~/tests/helpers/...`, `~/verify/...`, `~/scripts/...`, `~/eval/...`). Cross-module imports are allowed ONLY through the "public entry" listed per module (e.g. `@/lib/wechat-export`, `@/server/llm`, `@/server/extract`, `@/components/evidence`). Never deep-import another module's internals. One exception: `src/server/app.ts` (core) mounts each module's routers from `src/server/<module>/routes/*.ts` directly.
...
- Tests live inside the owning module's paths (always co-located `*.test.ts` next to the code under test — no `__tests__/` directories; shared test-only helpers are `test-*.ts` files in the module, e.g. `src/server/review/test-fixtures.ts`, `src/lib/wechat-export/test-synthetic.ts`). Vitest config (core) picks up `**/*.test.ts(x)` excluding `tests/e2e/`, `node_modules`, `.next`, `.open-next`.
...
- `src/app/layout.tsx`, `src/app/globals.css`, `src/app/providers.tsx`, `src/app/not-found.tsx`, `src/app/error.tsx`
- `src/app/(auth)/**` (sign-in, sign-up pages; **sign-up success redirects to `/welcome`**, sign-in success to `/`)
- `src/app/(app)/layout.tsx`, `src/app/(app)/error.tsx`, `src/app/(app)/loading.tsx`, `src/app/(app)/dev/layout.tsx` (calls `notFound()` unless `NEXTJS_ENV=development`; hosts module showcase pages, see §8)
- `src/app/api/[[...route]]/route.ts`
- `src/server/app.ts`, `src/server/auth.ts`, `src/server/auth.config.ts`, `src/server/env.ts`, `src/server/context.ts`, `src/server/errors.ts`, `src/server/middleware/**`
- `src/server/db/**` (schema, client, owner helpers), `drizzle/**` (generated migrations)
- `src/server/routes/_stub.ts`, `src/server/routes/me.ts`, `src/server/routes/settings.ts`, and the INITIAL 501 stub of every other module's `src/server/<module>/routes/<file>.ts` (after creation, ownership transfers to the owner in the table below)
- `src/contracts/**` (all zod schemas + inferred types)
- `src/lib/api-client.ts`, `src/lib/auth-client.ts`, `src/lib/query.ts`, `src/lib/time.ts`, `src/lib/pinyin.ts`, `src/lib/lunar.ts`, `src/lib/cn.ts`, `src/lib/links.ts`
- `src/components/ui/**` (shadcn primitives, Loam-themed), `src/components/loam/**` (typography, BlockBoundary, BlockError, Skeleton, Page shell), `src/components/topbar/**` (TopBar)
- `src/styles/**` (tokens)
...
- **Bootstrap placeholders** (created once by core in wave 1 so the app compiles and every link resolves; ownership = the named module from the start; the module overwrites them):
  - `src/components/import-overlay/index.tsx` (import), `src/components/search-overlay/index.tsx` (search), `src/components/person-picker/index.tsx` (search), `src/components/evidence/index.tsx` (review) — export exactly the §2.8 names/props with minimal behaviour (overlay hosts render nothing; `PersonPicker` = plain list of `candidates` + "新建人物"; `EvidenceRow` renders children; `EvidenceMark` renders the superscript number, no block).
  - Pages `src/app/(app)/page.tsx` (home), `src/app/(app)/p/[id]/page.tsx` (person), `src/app/(app)/imports/[id]/page.tsx` (import-result), `src/app/(app)/chats/[id]/page.tsx` (chat), `src/app/(app)/settings/page.tsx` (settings), `src/app/(app)/welcome/page.tsx` (home): each renders the Loam page shell with a serif title ("人物 #12", "导入 #3", …) and one line "这一页还在建设中", HTTP 200, no client fetches, no console output. Wave-2 overlays/scenarios navigate to these.
...
// src/lib/links.ts  — the ONLY way modules build in-app URLs
...
### 1.2 parser (wave 1)
**Owns**: `src/lib/wechat-export/**`, `scripts/parser/**`
...
### 1.4 import (wave 2)
**Owns**: `src/server/import/routes/imports.ts` (whole file), `src/server/import/routes/chats-list.ts`, `src/server/import/**` (incl. `routes/`), `scripts/import/**`, `src/components/import-overlay/**`, `src/app/(app)/dev/import/**`, `tests/e2e/import/**`, `verify/scenarios/import/**`
...
// src/server/import/staging.ts (import-internal, not a public entry)
...
1. Step 1: the overlay keeps the ZIP `File` object (not the unzipped bytes) in a **module-level store** `src/components/import-overlay/upload-store.ts`: `Map<importId, { file: File; sha256: string; names: string[] }>`, filled after `POST /api/imports` returns the import id. The store lives in JS module scope, so it survives the overlay closing and client-side route changes (`src/app/(app)/layout.tsx` never unmounts).
...
### 1.5 extract (wave 2)
**Owns**: `scripts/extract/**`, `src/server/extract/**` (incl. `src/server/extract/__fixtures__/**` hand-written test cassettes), `prompts/**`, `scripts/extract-offline.ts`
...
### 1.6 review (wave 2)
**Owns**: `src/server/review/routes/review.ts`, `src/server/review/routes/evidence.ts`, `src/server/review/routes/imports-review.ts`, `src/server/review/routes/people-actions.ts`, `src/server/review/**`, `src/components/evidence/**`, `src/app/(app)/dev/review/**`, `verify/scenarios/review/**`
...
### 1.7 person (wave 3)
**Owns**: `src/app/(app)/p/**`, `src/components/person/**`, `src/server/person/routes/people.ts`, `src/server/person/**`, `src/app/(app)/dev/person/**`, `verify/scenarios/person/**`
...
### 1.8 search (wave 2)
**Owns**: `src/server/search/routes/search.ts`, `src/server/search/routes/people-index.ts`, `src/server/search/**`, `src/components/search-overlay/**`, `src/components/person-picker/**`, `src/app/(app)/dev/search/**`, `verify/scenarios/search/**`
...
### 1.9 home (wave 3)
**Owns**: `src/app/(app)/page.tsx`, `src/app/(app)/_home/**`, `src/app/(app)/welcome/**`, `src/components/home/**`, `src/server/home/routes/home.ts`, `src/server/home/**`, `src/app/(app)/dev/home/**`, `verify/scenarios/home/**`
...
**Owns**: `src/app/(app)/imports/**`, `src/components/import-result/**`, `src/app/(app)/dev/import-result/**`, `verify/scenarios/import-result/**`
...
### 1.11 chat (wave 3)
**Owns**: `src/app/(app)/chats/**`, `src/components/chat/**`, `src/server/chat/routes/chats.ts`, `src/server/chat/**`, `src/app/(app)/dev/chat/**`, `verify/scenarios/chat/**`
...
**Owns**: `src/app/(app)/settings/**`, `src/components/settings/**`, `src/server/settings/routes/export.ts`, `src/server/settings/**`, `src/app/(app)/dev/settings/**`, `verify/scenarios/settings/**`
...
**Owns**: `fixtures/synthetic/**` (ZIPs + `*.intent.json`), `scripts/synthetic/**` (generator), `eval/src/**`, `prompts/judge/**`, `eval/tests/**` (hand-built mini gold + predictions for metric unit tests; not real gold), `eval/judge-cache/**`, `eval/reports/**`, `eval/runs/**` (gitignored scratch).
...
### 1.17 interaction (wave 5)
**Owns**: `src/server/interaction/**`, `src/server/interaction/routes/interaction.ts`, `src/components/interaction/**`, `src/app/(app)/dev/interaction/**`, `verify/scenarios/interaction/**`
**Responsibilities** (SPEC §7 交互层, §8.8, §9.5「来往」, §9.9「这次聊了什么」):
- Conversation aggregation: segments → conversations, pure, `groupSegments()`. Same 3 h gap rule as `planWindows`, from the single constant `SESSION_GAP_HOURS = 3` in `src/contracts/common.ts` (core); `src/server/extract/windowing.ts` and `src/server/interaction` both import it, neither redefines it.
- Rhythm derivation (pure over aggregated rows; never stored).
- Loop state derivation (`open | done | dropped` from `closedMessageId`/`closedReason`; `expired` computed at read time, never stored).
- `GET /api/people/:id/interaction`, `GET /api/imports/:id/interaction`, `POST /api/loops/:id/close`, `POST /api/loops/:id/reopen`, `PATCH /api/segments/:id`. Both GETs are registered before `people.ts` / `imports.ts` catch-alls.
- The 「来往」 section component, the infobox 最后一次聊天 line component, the import-result 「这次聊了什么」 block, and the home 约定 rows' data. Each is a **self-contained component mounted by the owning page**; interaction never edits those pages (the integrator does the mount).
**Public entries**:
- `@/server/interaction` → `getPersonInteraction(db, ownerId, personId)`, `getImportInteraction(db, ownerId, importId)`, `getUpcomingPlans(db, ownerId, days)`, `getUpcomingLoops(db, ownerId, days, today?)`, `searchInteraction(db, ownerId, q, limit)`, `closeLoop(db, ownerId, loopId, reason)`, `reopenLoop(db, ownerId, loopId)`, `patchSegment(db, ownerId, segmentId, patch)`, `groupSegments(segments)`, `deriveRhythm(input)`, `loopState(loop, today)`.
- `@/components/interaction` → `InteractionSection` (person page), `LastContactLine` (infobox), `ImportConversations` (import-result), `PlanRow` (home).
**Does not own**: writing segments/loops (that is extract, §6), the review verbs `confirm`/`reject` on loops (that is review's `applyReview` with `type: 'loop'`).

### Ownership of route files (core's live in `src/server/routes/`, every other module's in `src/server/<module>/routes/`; core created each as a 501 stub, then the owner owns it)

| file | owner | routes |
|---|---|---|
| `src/server/routes/me.ts` | core | `GET /api/me`, `GET /api/health` |
| `src/server/routes/settings.ts` | core | `GET /api/settings`, `PATCH /api/settings` |
| `src/server/import/routes/imports.ts` | import | `POST /api/imports/check`, `POST /api/imports`, `GET/DELETE /api/imports/:id`, `PUT /api/imports/:id/attachments/:name`, `POST /api/imports/:id/mapping`, `POST /api/imports/:id/jobs/next`, `POST /api/imports/:id/jobs/retry` |
| `src/server/import/routes/chats-list.ts` | import | `GET /api/chats` |
| `src/server/review/routes/imports-review.ts` | review | `GET /api/imports/:id/review` |
| `src/server/review/routes/review.ts` | review | `POST /api/review/bulk`, `POST /api/review/:type/:id` (bulk registered first) |
| `src/server/review/routes/people-actions.ts` | review | `POST /api/people`, `POST /api/people/:id/merge`, `POST /api/people/:id/split`, `POST /api/people/:id/claims`, `/dates`, `/relations`, `/events` |
| `src/server/search/routes/people-index.ts` | search | `GET /api/people` |
| `src/server/review/routes/evidence.ts` | review | `GET /api/evidence/:type/:id` |
| `src/server/person/routes/people.ts` | person | `GET/PATCH/DELETE /api/people/:id` |
| `src/server/chat/routes/chats.ts` | chat | `GET /api/chats/:id`, `GET /api/chats/:id/messages`, `GET /api/attachments/:id` |
| `src/server/home/routes/home.ts` | home | `GET /api/home` |
| `src/server/search/routes/search.ts` | search | `GET /api/search` |
| `src/server/settings/routes/export.ts` | settings | `GET /api/export`, `DELETE /api/data` |
| `src/server/interaction/routes/interaction.ts` | interaction | `GET /api/people/:id/interaction`, `GET /api/imports/:id/interaction`, `POST /api/loops/:id/close`, `POST /api/loops/:id/reopen`, `PATCH /api/segments/:id` |

Each router file default-exports a Hono sub-app built with **chained** calls (required for RPC type inference):
```ts
// src/server/home/routes/home.ts
...
```
`src/server/app.ts` (core) mounts under `/api` in this order: (1) Better Auth handler on `/auth/*`; (2) `Server-Timing` middleware; (3) **session middleware**: sets `c.var.env` (`parseServerEnv(getCloudflareContext().env)`, cached per isolate; invalid env → 500 `internal` with the variable name logged, never its value), resolves the session and sets `c.var.user` (or null) and `c.var.db`; for every path except `/api/auth/*` and `/api/health` it returns the 401 envelope when there is no user (so no route can forget auth); `requireUser(c)` returns the non-null user; (4) routers: me, settings, imports, chats-list, imports-review, review, people-actions, people-index, evidence, interaction, people, chats, home, search, export. Route files must not define paths owned by another file. More specific paths (`/people/:id/merge`, `/people/:id/interaction`) are registered in `people-actions` and `interaction` BEFORE `people`.

The 501 stub (core): `src/server/routes/_stub.ts` exports `notImplemented(c)` returning the error envelope with code `not_implemented`, status 501. Stubs declare every route with its zod validator so `AppType` is complete from day one.

### UI slots (core)
`src/components/topbar/TopBar.tsx` renders: left 产品名 link `/`; center `<SearchTrigger variant="topbar" />` (the component itself collapses to an icon under 640px); right `导入` button (`useImportOverlay().open()`) + account menu (设置 → `/settings`, 退出). Overlay hosts are mounted once in `src/app/(app)/layout.tsx`:
```tsx
<QueryProvider>
  <TopBar />
  <main>{children}</main>
...
All zod v4. Each file exports schemas `XxxSchema` and types `type Xxx = z.infer<typeof XxxSchema>`. `src/contracts/index.ts` re-exports all. Contracts import nothing from server/ or app/. (UI component prop types, §2.8, are TypeScript interfaces exported from the owning component module, not zod.)

### 2.1 Primitives — `src/contracts/common.ts`
```ts
...
### 2.2 Entity DTOs — `src/contracts/entities.ts` (API-facing; `ownerId` never serialized)
```ts
...
// interaction (SPEC §7 交互层)
SegmentDTO     { id, chatId, chatTitle, startSeq, endSeq, startedAt: MsgTime, endedAt: MsgTime, messageCount, summary: string, topics: string[], hidden: boolean, firstMessageId: Id, participants: PersonRefDTO[], importId: Id|null, sourceKind, createdAt }
ConversationDTO{ chatId, chatTitle, chatKind: ChatKind, startedAt: MsgTime, endedAt: MsgTime, messageCount, segments: SegmentDTO[], topics: string[] /* union, deduped, in order */, firstMessageId: Id }
LoopDTO        { id, personId, direction: 'mine'|'theirs'|'mutual', kind: 'promise'|'question'|'plan'|'request', text: string, dueAt: PartialDate|null, openedAt: MsgTime, openedMessageId: Id|null, closedAt: MsgTime|null, closedMessageId: Id|null, closedReason: 'done'|'dropped'|null, state: 'open'|'done'|'dropped' /* derived */, expired: boolean /* derived at read time, never stored */, daysOpen: number, status: Status, importId: Id|null, sourceKind, evidenceCount, createdAt }
RhythmDTO      { conversationCount: number, conversationCountThisYear: number, lastAt: MsgTime|null, daysSinceLast: number|null, medianGapDays: number|null /* null under MIN_RHYTHM_CONVERSATIONS=5 */, initiatedByMe: number|null, initiatedByThem: number|null /* both null for group-only contact */, privateOnly: boolean }
LlmCallDTO     { see §2.6 }
```
...
### 2.3 Parsed export — `src/contracts/parsed-export.ts` (produced by parser, uploaded by import)
```ts
...
### 2.4 API request/response — `src/contracts/api/*.ts` (one file per route group)
...
| `GET /api/home` | – | `HomeResponse { isEmpty: boolean, needsOnboarding: boolean, upcoming: { person: PersonRefDTO, dateId: Id, label: string, solar: 'YYYY-MM-DD', lunarLabel: string\|null, days: number }[], recentlyUpdated: { person: PersonRefDTO, latest: Pick<ClaimDTO,'id'\|'statement'\|'category'> }[], pinned: PersonRefDTO[], index: PeopleIndexResponse, recentImports: { id, chatTitle, dateFrom, dateTo, createdAt, status }[] /*last 5 with status ≠ 'mapping'*/ }` — `upcoming[]` rows gain `kind: 'date'\|'plan'\|'loop'`; dated matters carry `loopId`, `loopKind`, `direction`, `status` and original `dueAt` precision, dates carry `dateId` (the other id is null). All sort by `solar` asc and share the caller's timezone date (SPEC §9.4) |
| `GET /api/search` | `?q=string(1..100)&limit=20&types=all\|people\|claims` | `{ q, people: { person: PersonRefDTO, matchedAlias: string\|null }[], claims: { person: PersonRefDTO, claimId: Id, statement: string, highlights: [start: number, end: number][] }[], interaction: { kind: 'segment'\|'loop', id: Id, person: PersonRefDTO\|null, chatId: Id\|null, chatTitle: string\|null, at: MsgTime, text: string, href: string, highlights: [number, number][] }[] }` (`types` also accepts `interaction`) |
| `GET /api/imports/:id/interaction` | – | `{ conversations: ConversationDTO[] }` — the 「这次聊了什么」 block; conversations touching this import's segments, newest first |
| `GET /api/people/:id/interaction` | `?timeline=5` (conversations to return; `all` for the 更早 expansion) | `InteractionResponse { rhythm: RhythmDTO, loops: LoopDTO[] /* open + expired, oldest first; done/dropped excluded */, closed: LoopDTO[] /* for the person page 历史 */, conversations: ConversationDTO[], hasMore: boolean }` |
| `POST /api/loops/:id/close` | `{ reason: 'done'\|'dropped' }` | `{ loop: LoopDTO }` — also sets `status: 'confirmed'` when it was `proposed` (SPEC §7 交互层: closing is confirming). `closedMessageId` stays null (closed by hand, not by a message), `closedAt` = now |
| `POST /api/loops/:id/reopen` | – | `{ loop: LoopDTO }` — clears `closedMessageId`/`closedAt`/`closedReason` |
| `PATCH /api/segments/:id` | `{ summary?: string, hidden?: boolean }` | `{ segment: SegmentDTO }` — editing sets `sourceKind: 'manual'` |
...
ReviewItem =
 ...
 | { type: 'loop', item: LoopDTO }
ImportReviewResponse { import: ImportDTO, chat: ChatDTO|null, progress: Progress, sections: { person: PersonRefDTO & { isNew: boolean /*created by this import*/ }, newCount: number, newClaims: ReviewItem[], changes: ReviewItem[], aliasesAndRelations: ReviewItem[], dates: ReviewItem[], events: ReviewItem[], loops: ReviewItem[] }[], highConfidence: { type: 'claim', id: Id }[], highConfidenceCount: number, allHandled: boolean, empty: boolean }
...
//   Handles/relations/dates/events/loops have no confidence and are never included (use "本节全部确认"). DECISIONS A6.
// The 「这次聊了什么」 block (SPEC §9.9) is NOT part of this response and NOT part of allHandled/progress:
//   ImportConversations fetches GET /api/imports/:id/interaction → { conversations: ConversationDTO[] } (interaction module).

ProfileResponse { person: PersonDTO, aliases: { kind: HandleKind, items: HandleDTO[] }[], infobox: { relationToMe: {value: string, relationId: Id}|null, city: {value, claimId}|null, work: {value, claimId}|null, school: {value, claimId}|null, birthday: ImportantDateDTO|null, otherDates: ImportantDateDTO[], chats: { chat: Pick<ChatDTO,'id'|'title'|'kind'>, messageCount, lastMessageAt }[], lastContactAt: MsgTime|null }  /* the 最后一次聊天 摘要 line is fetched by LastContactLine from GET /api/people/:id/interaction, not from here */, sections: { category: Category, claims: ClaimDTO[] /*confirmed + proposed*/ }[], relations: RelationDTO[], events: EventDTO[], history: ClaimDTO[] /*superseded + outdated + edited-before, desc*/ }

ExportDump { exportedAt: IsoString, version: 1, user: {id, email, name}, settings: SettingsDTO, chats, imports, importMessages, messages, attachments /*metadata only, no bytes*/, persons, handles, relations, claims, claimMentions, events, eventParticipants, importantDates, conversationSegments, segmentParticipants, loops, evidence, extractionJobs /*incl. rawOutput*/, reviewLog, llmCalls /*incl. rawOutput*/ }   // arrays of raw rows, camelCase, ownerId omitted
```
...
### 2.5 Extraction output — `src/contracts/extraction.ts` (SPEC §8.6, strict)
```ts
...
ExtractionOutputSchema = z.object({
  ...
  // interaction (SPEC §8.8); all three default to null/[] so extract.v1–v8 output parses unchanged
  segment:    z.object({ summary: z.string().min(4).max(300), topics: z.array(z.string().min(1).max(20)).max(6).default([]), speakers: z.array(PersonRef).default([]), evidence: Evidence }).strict().nullable().default(null),
  loops:      z.array(z.object({ person: PersonRef, direction: z.enum(['mine','theirs','mutual']), kind: z.enum(['promise','question','plan','request']), text: z.string().min(2).max(200), dueAt: z.string().max(20).optional(), evidence: Evidence }).strict()).default([]),
  closes:     z.array(z.object({ loopId: Id, reason: z.enum(['done','dropped']), evidence: Evidence }).strict()).default([]),
}).strict()
DedupOutputSchema = z.object({ duplicates: z.array(z.object({ newIndex: z.number().int(), existingClaimId: Id })).default([]) }).strict()
```
Relation `type` vocabulary (prompt-guided, not enforced): `parent|child|spouse|sibling|relative|friend|colleague|classmate|service_provider|client|other`; `label` holds the Chinese term ("外公"). **Direction: a relation reads `from` 是 `to` 的 `type`（例：from=外公, to=外孙, type=parent, label=外公）; `label` names `from` as seen from `to`, in the same reading** (core request eval-synthetic#2; seed, extract prompt, gold and person module all use this). Date `kind` vocabulary: `birthday|anniversary|memorial|other`. Loop `direction` has exactly one meaning — **whose move is next** — written from the user's side: `mine` = the user owes it (including answering a question THEY asked), `theirs` = the other person owes it, `mutual` = a shared plan. A `plan` is normally `mutual`. `text` stores the thing itself with **no subject** ("周五前把报价发过来"); the subject and verb are composed at render time by `loopSentence` in `src/lib/loop-text.ts`, shared by the person page and the import result page, so re-wording is a presentation change and never a re-extraction. Invalid-JSON / validation-failed / item-level evidence-out-of-window are distinct outcomes (§6).

### 2.6 LLM call record — `src/contracts/llm.ts`
```ts
...
- `src/server/app.ts` exports `type AppType = typeof app` (the chained app).
...
```
- `src/lib/query.ts`: `queryKeys` factory — `queryKeys.home()`, `.person(id)`, `.peopleIndex()`, `.importDetail(id)`, `.importReview(id)`, `.chat(id)`, `.chatMessages(id, params)`, `.evidence(type, id)`, `.search(q, types)`, `.settings()`, `.me()`. Modules may add private keys only prefixed with their module name (`['person', ...]`); shared ones must use the factory so invalidation works across modules.
- Usage:
```ts
...
- Input validation: every route uses core's `validator(target, schema)` (`src/server/middleware/validate.ts`, wraps `@hono/zod-validator`) with the contracts schema; failure → 400 envelope `validation_failed`.
...
- **Owner scoping** (enforced helper, core, `src/server/db/owned.ts`):
```ts
...
`auth.ts` (Better Auth, generated with `npx auth@1.7.5 generate --config src/server/auth.config.ts --adapter drizzle --dialect sqlite --output src/server/db/schema/auth.ts`, then committed): `user`, `session`, `account`, `verification`.
...
- `evidence` (link): owner_id, target_type (`handle|relation|claim|event|date|segment|loop`), target_id int, message_id fk messages cascade, created_at; **pk (target_type, target_id, message_id)**, no surrogate id; idx (owner_id, message_id)

`interaction.ts` (SPEC §7 交互层)
- `conversation_segments`: chat_id fk chats cascade, start_seq int, end_seq int, started_at text (MsgTime), ended_at text (MsgTime), message_count int, summary text, topics text(json string[]), hidden int bool not null default 0, import_id int null fk imports (set null), job_id int null, source_kind. **unique (owner_id, chat_id, start_seq, end_seq)** (re-extracting the same span overwrites, SPEC §8.8); idx (owner_id, chat_id, started_at), idx (owner_id, import_id)
- `segment_participants` (link): owner_id, segment_id fk conversation_segments cascade, person_id fk persons cascade, message_count int, created_at; pk (segment_id, person_id); idx (owner_id), idx (person_id, segment_id)
- `loops`: person_id fk persons cascade, direction text ('mine'|'theirs'|'mutual'), kind text ('promise'|'question'|'plan'|'request'), text text, text_norm text, due_at text null (PartialDate), opened_message_id fk messages **(set null)**, opened_at text (MsgTime), closed_message_id int null fk messages (set null), closed_at text null (MsgTime), closed_reason text null ('done'|'dropped'), status (proposed|confirmed|rejected), import_id null fk imports (set null), job_id null, source_kind. idx (owner_id, person_id, status), idx (owner_id, import_id), idx (owner_id, status, text_norm), idx (owner_id, due_at)
- `opened_message_id` is `set null` rather than cascade so deleting the opening message does not silently drop the item; `deleteImport` (§11) removes the loop explicitly via its evidence rows, like every other derived item.
- **No conversation table.** Conversations are `groupSegments()` over `conversation_segments` (§1.17). No `state` column on `loops`: open/done/dropped is derived from `closed_message_id`/`closed_reason`, `expired` from `due_at`/`opened_at` at read time (SPEC §7 交互层).
...
- `src/server/db/client.ts`: `getDb(): Db` = `drizzle(getCloudflareContext().env.DB, { schema })`, wrapped in React `cache()` for RSC; Hono uses `c.var.db` set in the session middleware per request. No module-level db singletons. `createDb(d1: D1Database): Db` exported for CLI/tests.
...
## 5. LLM adapter (`src/server/llm`, owned by llm)
...
- Dirs: `fixtures/cassettes/synthetic/` (committed), `fixtures/cassettes/real/` (gitignored), module test dirs (e.g. `src/server/extract/__fixtures__/cassettes/`). The caller chooses: eval passes explicitly by source; in-app uses `LLM_CASSETTE_DIR`.
...
## 6. Extraction pipeline (`src/server/extract`, owned by extract)
...
export function promptFeatures(version: string): { packMaxMessages: number | null; gapMarkers: boolean; milestoneRules: boolean }   // src/server/extract/prompt-version.ts; v1–v3 → {null,false}, v4+ → {40,true}; milestoneRules true from extract.v7; extract.v8 (registered, not PROMPT_VERSION) = v7's features

// prompt — pure
export const PROMPT_VERSION: string  // src/server/extract/prompt-version.ts, matches prompts/extract.v<N>.md front-matter
...
export interface WindowInput {
  ...
  known: { personId: number; label: string; handles: { kind: HandleKind; value: string }[]; claims: { id: number; statement: string; category: Category }[]
           // interaction (SPEC §8.8 input side), added from extract.v9:
           lastContact?: { at: MsgTime; summary: string } | null
           openLoops?: { id: number; kind: 'promise'|'question'|'plan'|'request'; direction: 'mine'|'theirs'|'mutual'; text: string; openedAt: MsgTime }[] }[]
}
// validation — pure
...
export interface OfflineExtractionResult {
  ...
  // interaction (SPEC §8.8). This is the shape `memoryStore` actually returns — the eval harness reads these keys
  // verbatim, so a rename here silently zeroes every interaction metric (core request eval-synthetic#5, DECISIONS I14).
  segments:  { startIdx: number; endIdx: number; startedAt: MsgTime; endedAt: MsgTime; messageCount: number; summary: string; topics: string[]; participants: { person: string; messageCount: number }[]; evidence: number[]; windowIndex: number }[]
  loops:     { person: string; direction: 'mine'|'theirs'|'mutual'; kind: 'promise'|'question'|'plan'|'request'; text: string; dueAt?: string; openedAt: MsgTime; openedIdx: number; closedIdx: number | null; closedAt: MsgTime | null; closedReason: 'done'|'dropped' | null; evidence: number[]; windowIndex: number }[]
  closes:    { loopIndex: number /* index into `loops`, as a claim's `supersedes` indexes `claims` */; reason: 'done'|'dropped'; evidence: number[]; windowIndex: number }[]
...
**Interaction extraction** (SPEC §8.8) runs as a **second, independent model call** issued in parallel with the extraction call over the same window — `Promise.all`, not sequential, because neither needs the other's output, so the window's wall time stays `max(extract, interaction)` rather than their sum.

It was originally three extra sections on the extraction output (`prompts/extract.v9.md`). Measured live on the same 4 synthetic zips against the same frozen gold, that cost the extraction its gates — claims P 0.90 → 0.76, claims R 0.77 → 0.66, handles P 1.00 → 0.875, transactionalAsClaimRatio 0.04 → 0.073 — while claims *count* rose 50 → 55, i.e. the model was not routing momentary content away, it was doing its original job worse with three more tasks in the same call. `extract.v9` is therefore **retired, not promoted**: `PROMPT_VERSION` is `extract.v8` (recorded live 2026-09-16, passes every synthetic gate) and the interaction call has its own `INTERACTION_PROMPT_VERSION = 'interaction.v1'` (`prompts/interaction.v1.md`) and its own `InteractionOutputSchema` (§2.5). DECISIONS I15/I17; overturns I6, whose "no second call" argument was about latency, not quality.

```ts
export const INTERACTION_PROMPT_VERSION: string
export function renderInteractionPrompt(input: WindowInput, version?: string): LlmMessage[]
export function validateInteraction(json: unknown, input: WindowInput): { output: InteractionOutput; dropped: DroppedItem[]; rawItemCount: number } | { error: 'validation_failed'; issues: string[] }
```
The interaction prompt gets a **smaller** input than the extraction prompt: window messages, person labels, `lastContact` and `openLoops` — but not the known persons' confirmed `claims` list, which it has no use for. That keeps the added cost well under a doubling.

**Failure isolation**: the two calls fail independently. An interaction failure is non-fatal exactly like a dedup failure — the window still counts as `done`, its claims still land, and `WindowOutcome.interaction` records `'ok' | 'skipped_deadline' | 'failed' | 'not_needed'` (the last for an empty window where no call was made, mirroring `dedup`).

**Attempt clock with two concurrent calls**: `WindowOutcome.latencyMs` still *sums* the calls — that is what they cost — but `attemptMs`, which feeds `p95WindowMs`, is `max(wall)` / `max(latency)` across the pair. Summing two calls that ran at the same time would charge the window twice for time it spent once; in replay that is not an approximation but simply wrong, and `p95WindowMs` is a wall-clock budget (§10). DECISIONS extract X45. The reverse is not true: if the extraction call fails the window is retried as before, and the interaction result of that attempt is discarded with it.

**Deadline** (the §6 budget, re-sliced): extract and interaction start together with `timeoutMs = min(20_000, deadlineAt - now - 6_000)`; dedup still runs only if `deadlineAt - now >= 4_000`; persistence keeps its ≥ 2 s. Interaction is skipped (not failed) when less than 6 s remain when it would start. `validateOutput` additions, all behind `opts.interaction`:
- `segment`/`loops`/`closes` evidence must be in-window (`evidence_out_of_window`) and must not rest only on context messages (`context_only`).
- `closes[].loopId` must appear in some `known[].openLoops` of this window's input, and the loop's `openedAt` must be `<=` the earliest close-evidence message's `sentAt`; otherwise `unknown_close`.
- `segment.summary` shorter than 4 chars, or with no non-context evidence, → the whole `segment` is dropped (`invalid_item`); at most one segment per window (extras dropped).
- a `loop` whose text carries no verb-like commitment marker and whose `kind` is `promise` is NOT rejected here — recall over precision is the prompt's job; the drop reasons stay mechanical.
`ResolvedItems` gains `segment?: { chatId; startSeq; endSeq; startedAt; endedAt; messageCount; summary; topics: string[]; participants: { personId; messageCount }[]; messageIds: number[] }` and `loops` / `closes`. `proposeItems` upserts the segment on `(ownerId, chatId, startSeq, endSeq)`, inserts loops, and applies closes as `UPDATE loops SET closed_message_id, closed_at, closed_reason WHERE id = ? AND owner_id = ? AND closed_message_id IS NULL`. Loop dedup rides the existing per-window dedup call (one call, keyed by person, candidates = that person's open loops of this import).
`d1Store.loadWindow` fills `lastContact` (latest `conversation_segments` row of any chat this person spoke in, strictly before the window's first message) and `openLoops` (that person's loops with `closed_message_id IS NULL`, `status != 'rejected'`, cap 12, newest first). `memoryStore` (eval) fills both from what the offline run itself has produced so far, so offline and in-app see the same shape.
...
- `eval/reports/synthetic/<timestamp>.json` (per-item detail, synthetic text only) and `eval/reports/summary/<timestamp>.json` (combined, **aggregate numbers only, no statements/names**) — committed; `eval/reports/real/<timestamp>.json` (per-item detail) — gitignored. `eval/reports/compare/compare-<vA>-vs-<vB>-<timestamp>.json` — aggregate only.
...
GoldFile {
  ...
  // interaction (SPEC §8.8; goldVersion 2 — additive, existing v1 files stay valid and score exactly as before)
  loops?:    { id: string, person: string, direction: 'mine'|'theirs'|'mutual', kind: 'promise'|'question'|'plan'|'request', text: string, dueAt?: string, evidence: number[] /*the message that opens it*/, closedBy?: number /*idx of the message that closes it, if any*/, closedReason?: 'done'|'dropped', optional?: boolean }[],
  conversations?: { id: string, startIdx: number, endIdx: number, topics: string[] /*the topics a summary must cover*/, optional?: boolean }[],
...
- **loops**: same person key AND judge step A over `text` (same prompt, `same`/`less_specific`/`different`), one-to-one greedy. `kind`/`direction` mismatch on a matched pair is reported (`loopKindMismatch`), not a miss. A gold loop with `closedBy` must additionally be closed by the prediction at that idx to count for `loopCloseRecall` (deterministic, reported separately from the loop TP).
- **conversations**: deterministic span match — a predicted conversation (`groupSegments` over the run's segments) matches a gold conversation when their `[startIdx, endIdx]` spans overlap by ≥ 50% of the gold span. Summary quality is judged only as topic coverage: fraction of `topics[]` whose string appears (normalized substring) in the matched conversation's joined summary + topics. Reported, not judged by LLM — cheap and stable.
...
**Step A — match** (`prompts/judge/judge-match.v1.md`), one call per (zip, person with ≥1 gold or predicted claim), and one per zip for events:
```ts
...
- **`predicted` is always what the run produced; only the ratios may be null.** A gold file that does not annotate a type cannot say whether an item is right, so that type gets no TP/FP/FN, no judge call, and null precision/recall — but its **count is still reported**. `TypeMetrics` carries `scored` next to `predicted`: `scored` is the precision denominator, `predicted` is the truth. They differ only where gold lacks the type. Reporting 0 predicted would be the mirror-image lie ("the model got every one wrong") and would fail a gate on a number nobody measured (DECISIONS I16, E22).
- Per type (`claims`, `relations`, `handles`, `dates`, `events`, `loops`):
...
- **Interaction metrics** (reported for every run; gated as noted):
  - `loops`: `precisionLenient` / `recallStrict` exactly as claims. **Gated: precision ≥ 0.80** — a wrong "you promised X" is worse than a missed one, but this is a new capability, so recall is reported, not gated, in the first loop; raise both once two eval runs exist.
  - `loopCloseRecall` = gold loops with `closedBy` that the run actually closed / gold loops with `closedBy`. Reported. It reads `loops[].closedIdx` — the resolved state the app would store in `loops.closed_message_id`, i.e. what a user would actually see — not the raw `closes[]`, which still contains closes that validation later threw away.
  - `loopFalseClose` counts **only** mechanically impossible closes, read from `closes[]`: (a) `loopIndex` is not a loop of this run, (b) the closing window is earlier than the window that first produced the loop, so it cannot have been shown it, or (c) the closing evidence is not strictly after the opening message. `validateOutput` already drops all three, so a survivor is a pipeline or harness bug and the **gate = 0** is safe; `details.interaction.falseCloses` names which of the three fired. A close of a loop that gold leaves open is a *model* mistake, not a mechanical one: it costs loop precision against the 0.80 gate and is **not** counted here. (Corrected 2026-09-16 — the first wording gated every disagreement with gold at zero, so one honest model mistake would have failed the whole source. DECISIONS I13, core request eval-synthetic#6.)
  - `conversationCoverage` = matched gold conversations / gold conversations (non-optional). Reported; warning below 0.8.
  - `topicCoverage` = mean topic coverage over matched conversations. Reported.
  - `segmentInvalidEvidence` — same rule and **gate = 0** as `invalidEvidencePost`, counted over segments and loops.
  - `sensitiveInStatement` extends to loop `text` and segment `summary`/`topics`. Same gate = 0.
...
- Report (`EvalReport`):
```ts
{ runId, createdAt, promptVersion, interactionPromptVersion: string|null /* observed from the calls the run made, not declared */, model, mode, deadlinePolicy: 'app'|'none', parserVersion, judgePromptVersions: { match: string, fp: string }, judgeFallback: boolean, aborted: string|null,
  usage: { inputTokens, outputTokens, calls, judgeInputTokens, judgeOutputTokens, byCall: Record<'extract'|'interaction'|'dedup'|'other', { inputTokens, outputTokens, calls, failedCalls }> /* bucketed by PROMPT VERSION, not `purpose`; the split's cost must be readable, and a second call must not be able to fall out of the cost report silently */ },
  ...
  zips: { zip, source, status: 'scored'|'gold_mismatch'|'no_gold', messageCount, windows: { total, failed, p95Ms, dedupSkipped, interactionFailed, interactionSkipped } /* without the last two, a failed second call and a model that found nothing both read as 0 loops */, metrics: TypeMetrics, errors: Record<'claims'|'relations'|'handles'|'dates'|'events', Record<ErrorCode, number>>, details?: { tp: [], fp: [], fn: [] } /* only in per-source files */ }[] }
TypeMetrics = Record<'claims'|'relations'|'handles'|'dates'|'events'|'loops', { predicted, scored, tpStrict, tpLenient, fp, fn, precisionStrict, precisionLenient, recallStrict, recallLenient, yieldPer100 }> & { transactionalAsClaimRatio, sensitiveInStatement, invalidEvidencePost, invalidEvidencePreRate, evidenceOverlap }
SourceMetrics = TypeMetrics & { zips: number, messages: number }
```
Combined report `eval/reports/summary/<timestamp>.json` contains no per-item text; real zip names replaced by `real-1..n` (sorted by basename); `gold[].zip` likewise anonymized. `--compare vA` writes `eval/reports/compare/compare-<vA>-vs-<vB>-<timestamp>.json` with side-by-side `sources.*` metrics and deltas (both runs must share identical `gold[].goldSha256`, else refused), plus `cost A / cost B / Δ` per `usage.byCall` bucket, and prints a table. **Interaction-version guard**: two *different* non-null `interactionPromptVersion`s is a refusal like a gold-sha mismatch (two prompts moved at once, no row is attributable); `null` on one side is the two-call split itself being measured (I17), a legitimate comparison, so it is a loud warning instead. The table carries `claimsVsInteraction` rows and **no 分流 narrative**: since I17 the extraction and interaction calls are independent and the extraction call is not told the interaction layer exists, so a claims change is a claims change — segments or loops rising neither explains nor offsets a claims precision drop. The earlier wording said the opposite and was an excuse waiting for the regression it got (I15, E22). Orchestrator copies `latestReport` + `passed` into STATUS.json.
- Top-up eval (extract-owned, DECISIONS ## extract X27): `node --import tsx scripts/extract/eval-fill.ts` replays the current prompt version's extract cassettes, refuses live extract calls, and records only dedup/judge misses (`--max-live-tokens` caps usage); for pipeline changes that keep extract requests unchanged. A plain `pnpm eval` replay afterwards must show 0 cassette misses. `--allow-extract-misses` also records extract calls, only on cassette misses of the current prompt version (a rule that changes new persons changes later windows' known-person lists, hence their requests). Registered-but-not-current versions (extract.v7, extract.v8) are evaluated with `--prompt`.
...
```
`seed.person(tag)`, `seed.import(tag)`, `seed.chat(tag)`, `seed.claim(tag)`. Component showcases that are not a real page render at `/dev/<module>/<name>` (module-owned `src/app/(app)/dev/<module>/**`, 404 outside development).
...
| P1b | Parser unit bench (Node) ≤ 1.5 s for same zip | vitest bench in `src/lib/wechat-export` | parser | parser critic |
...
**Delete import** — owner: **import** (`src/server/import/delete.ts`, `deleteImport`), route `DELETE /api/imports/:id`:
...
7. **Interaction (SPEC §7 删除语义)**: segments whose every evidence message ⊆ M are deleted (with their `segment_participants` and evidence); segments keeping some evidence have `message_count`, `started_at`, `ended_at` recomputed from the messages still in `[start_seq, end_seq]`, and are deleted if that count reaches 0. Loops follow step 2 like any derived item. Additionally, **every loop whose `closed_message_id` ∈ M is reopened**: `closed_message_id`, `closed_at`, `closed_reason` set null — the sentence that closed it is gone, so the item is open again. `segment_participants` rows of persons with no remaining message in the segment are dropped.
Executed as ordered `db.batch` chunks; idempotent if interrupted.

**Delete person** — owner: **person** (`src/server/person/delete.ts`, `deletePerson`), route `DELETE /api/people/:id`: set `messages.sender_handle_id` null for its handles (sender shows raw `sender_name`), delete handles, claims, important_dates, relations (both directions), event_participants rows (events left with 0 participants deleted), claim_mentions, **loops and their evidence**, **`segment_participants` rows** (segments themselves are kept — a conversation belongs to a chat, not a person), evidence rows of all deleted items, avatar R2 object; reviewLog rows kept. Persons with `merged_into_id = id` → deleted too. Self person cannot be deleted (409 `conflict`).
...
**Merge** (review): moves handles, claims, dates, relations (dropping self-loops and exact duplicates, merging evidence), event_participants, claim_mentions, **loops and `segment_participants` (summing `message_count` on a pk collision)** from `from` to `into`; `from.merged_into_id = into`; `GET /api/people/:from` returns `redirectTo`.
...
| 1b (parallel with 1a) | parser, llm, eval-synthetic | contracts (core lands `src/contracts/**` first; until then code against §2 locally and reconcile) | parser tests; llm replay tests + ≤5 live smoke calls; synthetic ZIPs + intent files; harness + metric unit tests green on `eval/tests` mini fixtures with `fakeJudge` and a stub extractor; annotation tools work |
...
| 5 | **interaction**; extract (round: prompt v9 + §8.8 validation/store); eval-synthetic (gold + metrics for segments/loops); annotator (gold extension) | 1–4 | interaction backend + UI critics; extract eval gates hold for claims/handles/relations **and** the §7.4 interaction metrics; SPEC M7 acceptance (reverse-order import, delete-reopen) green |
...
| interaction (5) | `@/components/evidence`, `@/server/db`, `@/lib/links`, `@/lib/time`, `@/contracts` | review (2), core (1) |
| person (3) | `@/components/interaction.InteractionSection`, `LastContactLine` | interaction (5) — mounted by the integrator in wave 5 |
| import-result (3) | `@/components/interaction.ImportConversations` | interaction (5) — mounted by the integrator in wave 5 |
| home (3) | `@/server/interaction.getUpcomingLoops`, `@/components/home/TasksPanel` | interaction (5) — mounted by the integrator in wave 5 |
| search (2) | `@/server/interaction.searchInteraction` | interaction (5) — wired by the integrator in wave 5 |
| extract (2, wave-5 round) | `@/contracts` interaction output schema | core (1) |
...
- **interaction** — backend critic: contract §2.4, typecheck, tests, two-account isolation, `groupSegments`/`deriveRhythm`/`loopState` unit tests (boundary drift: two adjacent imports merging into one conversation; reverse-order import; delete-import reopen), rhythm suppressed under 5 conversations, no `state` column anywhere; plus UI critic (design director) on `interaction/showcase` — 「来往」 with all three blocks, rhythm-only, loops-only, expired loop, unconfirmed loop, empty (section absent), long timeline, 390 width — and the SPEC §9.3 ban list (no counts, no badges, no global list).
