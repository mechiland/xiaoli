...
人情账本（导出中转账红包无金额）、语音识别（导出中语音只有时长）、与用户无关的事务信息、提醒推送（v1 在首页和事项页展示近期日期与待办）、移动端分享扩展、多租户与计费。
...
**loops**（未结事项）：`id`, `personId`, `direction`（mine | theirs | mutual）, `kind`（promise | question | plan | request）, `text`, `dueAt`（PartialDate，可空）, `openedMessageId`, `openedAt`, `closedMessageId`（可空）, `closedAt`（可空）, `closedReason`（done | dropped，可空）, `status`（proposed | confirmed | rejected）, `importId`, `sourceKind`

`direction` 只有一个含义：**下一步该谁动**，从我这边写。`mine` = 该我做、该我回答；`theirs` = 该对方做、该对方回答；`mutual` = 两个人一起的约定。

包括：我答应了对方什么（`promise` + `mine`）、对方答应了我什么（`promise` + `theirs`）、对方问了我而我没回（`question` + `mine` —— 欠回答的是我，所以是 `mine`）、我问了对方而对方没回（`question` + `theirs`）、约好但还没发生的事（`plan` + `mutual`，带 `dueAt`），以及明确要求我处理的事情（`request` + `mine`，如老师要求提交报告）。
...

{
  ...
  loops:      { person: PersonRef; direction: 'mine'|'theirs'|'mutual';
                kind: 'promise'|'question'|'plan'|'request'; text: string; dueAt?: string;
                evidence: number[] }[]
...
- 每条输出必须有 evidence。
- 未结事项只记原文说出口的承诺、提问、约定和请求，不记"应该会""可能"。对方问了而后文有人答了的，不是未结事项。
...
- `loops`：这个窗口里新出现的未结事项。必须是原文明确说出口的承诺、提问、约定或请求，不推断意图。
...
1. 原文明确说出口的承诺、提问、约定或行动要求，不推断意图。
2. 对话结束时仍未完成、回答或取消。
3. 有后续行动或关系上的价值。老师要求明天带衣服、周五前交报告，即使只重要一天，也值得保留；长期人物事实仍遵循 §8.7 的标准。

`request` 表示尚未完成的明确行动要求，不以用户先承诺为前提。群聊必须明确适用于用户或全群；面向其他孩子或特定人群、无法确认用户适用的要求不记。请求关联提出要求的人；`direction` 仍表示下一步该谁行动。通知、承诺、提问和约定都可携带 `dueAt`，保留原文的日期精度。

“收到”“好的”不能证明提交报告、带材料等行动完成；其他家长完成也不代表用户完成。此规则进入 `interaction.v3`，校验层额外拦截仅以回执为证据的行动关闭。`extract.v8` 的人物抽取保持独立。旧版提示词和评测金标准保留，用于对照；旧交互质量数字不代表新请求场景的质量。
...
采用 Wikipedia 式的正文与右侧信息栏。左上角菜单打开导航抽屉，提供主页 `/`、人物 `/people`、事项 `/tasks` 和设置入口。菜单支持键盘、Escape 关闭与焦点返回。人物和事项是互相关联的独立视图。

顶栏在所有页面一致：左侧菜单按钮和产品名（回首页），中间搜索入口，右侧"导入"按钮和账户菜单（设置、退出）。
...
不使用红点、催促通知、连续使用天数或按久未联系排序的名单。事项视图可以显示时间范围内的条数；未确认事项标注“待确认”，点击回到人物页核对原文。推送提醒仍不在本轮范围内。
...
- **右侧事项栏**：生日和重要日期，以及带 `dueAt` 的未结承诺、提问、约定、请求，按日期混排。可切换未来 7 天 / 30 天（含今天与边界日），默认 7 天；侧栏桌面最多展示 6 项，窄屏最多 3 项，链接到独立事项页查看完整范围。日期保留农历标注，只有月份时显示“某月内 · 具体日期未定”。行动事项支持完成与撤销，生日不提供完成按钮。无事项时展示空状态。窄屏右栏移到人物内容上方。
...
来往这一节不做"补充"入口：它描述的是已经发生的对话，手工添加一次不存在的来往没有意义。本轮不新增手动录入待办入口；已抽取的待办同时出现在人物页与事项视图。
