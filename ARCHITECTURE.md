...
**Owns**: `server/interaction/**`, `server/routes/interaction.ts`, `components/interaction/**`, `app/(app)/dev/interaction/**`, `verify/scenarios/interaction/**`
**Responsibilities** (SPEC §7 交互层, §8.8, §9.5「来往」, §9.9「这次聊了什么」):
- Conversation aggregation: segments → conversations, pure, `groupSegments()`. Same 3 h gap rule as `planWindows`, from the single constant `SESSION_GAP_HOURS = 3` in `contracts/common.ts` (core); `server/extract/windowing.ts` and `server/interaction` both import it, neither redefines it.
...
| `server/routes/interaction.ts` | interaction | `GET /api/people/:id/interaction`, `GET /api/imports/:id/interaction`, `POST /api/loops/:id/close`, `POST /api/loops/:id/reopen`, `PATCH /api/segments/:id` |
