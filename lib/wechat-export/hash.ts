import type { MessageKind } from '@/contracts'

const encoder = new TextEncoder()

/** FNV-1a 64-bit over the UTF-8 bytes of `s`, as 16 lowercase hex chars. Sync, no BigInt (16-bit limbs). */
export function fnv1a64Hex(s: string): string {
  const bytes = encoder.encode(s)
  // offset basis 0xcbf29ce484222325, little-endian 16-bit limbs
  let h0 = 0x2325
  let h1 = 0x8422
  let h2 = 0x9ce4
  let h3 = 0xcbf2
  for (let i = 0; i < bytes.length; i++) {
    h0 ^= bytes[i]
    // h *= 0x100000001b3  ==  h * 0x1b3 + (h << 40)
    const t0 = h0 * 0x1b3
    let t1 = h1 * 0x1b3
    let t2 = h2 * 0x1b3 + (h0 << 8)
    let t3 = h3 * 0x1b3 + (h1 << 8)
    t1 += t0 >>> 16
    t2 += t1 >>> 16
    t3 += t2 >>> 16
    h0 = t0 & 0xffff
    h1 = t1 & 0xffff
    h2 = t2 & 0xffff
    h3 = t3 & 0xffff
  }
  return hex4(h3) + hex4(h2) + hex4(h1) + hex4(h0)
}

function hex4(n: number): string {
  return n.toString(16).padStart(4, '0')
}

/**
 * WeChat names exported media `微信图片_<YYYYMMDDHHMM>_<n>.<ext>` / `微信视频_…` from the EXPORT time, with `n` counting
 * through the export, so a later export of the same chat renames every image/video (DECISIONS parser P14).
 */
const GENERATED_MEDIA_NAME_RE = /微信(图片|视频)_\d{8,14}(?:_\d+)?\.[A-Za-z0-9]{1,5}/g

/** Body as fingerprinted: image/video bodies with generated media file names replaced by a constant; others unchanged. */
export function fingerprintBody(kind: MessageKind, body: string): string {
  if (kind !== 'image' && kind !== 'video') return body
  return body.replace(GENERATED_MEDIA_NAME_RE, '微信$1_*')
}

/**
 * ARCHITECTURE §1.2: FNV-1a 64 hex of `${senderName}${sentAt}${kind}${fingerprintBody(kind, body)}`. For image/video,
 * a generated media file name counts as a constant, so re-exports align.
 */
export function fingerprint(m: { senderName: string; sentAt: string; body: string; kind: MessageKind }): string {
  return fnv1a64Hex(`${m.senderName}${m.sentAt}${m.kind}${fingerprintBody(m.kind, m.body)}`)
}

function getSubtle(): SubtleCrypto | null {
  const c = (globalThis as { crypto?: Crypto }).crypto
  return c?.subtle ?? null
}

function toHex(out: Uint8Array): string {
  let s = ''
  for (let i = 0; i < out.length; i++) s += out[i].toString(16).padStart(2, '0')
  return s
}

/**
 * SHA-256 hex via WebCrypto. Falls back to a pure-JS implementation when `crypto.subtle` is unavailable
 * (insecure http origins other than localhost, e.g. a dev server opened by LAN IP).
 */
export async function sha256Hex(bytes: Uint8Array): Promise<string> {
  const subtle = getSubtle()
  if (!subtle) return sha256HexJs(bytes)
  return toHex(new Uint8Array(await subtle.digest('SHA-256', bytes as unknown as BufferSource)))
}

const K = new Int32Array([
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5, 0xd807aa98, 0x12835b01,
  0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174, 0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc,
  0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da, 0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147,
  0x06ca6351, 0x14292967, 0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070, 0x19a4c116, 0x1e376c08,
  0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3, 0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208,
  0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2,
])

/** Pure-JS SHA-256 (exported for tests). */
export function sha256HexJs(data: Uint8Array): string {
  const H = new Int32Array([0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19])
  const W = new Int32Array(64)
  const block = (buf: Uint8Array, off: number) => {
    for (let i = 0; i < 16; i++) {
      const j = off + i * 4
      W[i] = (buf[j] << 24) | (buf[j + 1] << 16) | (buf[j + 2] << 8) | buf[j + 3]
    }
    for (let i = 16; i < 64; i++) {
      const w15 = W[i - 15]
      const w2 = W[i - 2]
      const s0 = ((w15 >>> 7) | (w15 << 25)) ^ ((w15 >>> 18) | (w15 << 14)) ^ (w15 >>> 3)
      const s1 = ((w2 >>> 17) | (w2 << 15)) ^ ((w2 >>> 19) | (w2 << 13)) ^ (w2 >>> 10)
      W[i] = (s1 + W[i - 7] + s0 + W[i - 16]) | 0
    }
    let a = H[0], b = H[1], c = H[2], d = H[3], e = H[4], f = H[5], g = H[6], h = H[7]
    for (let i = 0; i < 64; i++) {
      const t1 =
        (((e >>> 6) | (e << 26)) ^ ((e >>> 11) | (e << 21)) ^ ((e >>> 25) | (e << 7))) + ((e & f) ^ (~e & g)) + h + K[i] + W[i]
      const t2 = (((a >>> 2) | (a << 30)) ^ ((a >>> 13) | (a << 19)) ^ ((a >>> 22) | (a << 10))) + ((a & b) ^ (a & c) ^ (b & c))
      h = g
      g = f
      f = e
      e = (d + t1) | 0
      d = c
      c = b
      b = a
      a = (t1 + t2) | 0
    }
    H[0] = (H[0] + a) | 0
    H[1] = (H[1] + b) | 0
    H[2] = (H[2] + c) | 0
    H[3] = (H[3] + d) | 0
    H[4] = (H[4] + e) | 0
    H[5] = (H[5] + f) | 0
    H[6] = (H[6] + g) | 0
    H[7] = (H[7] + h) | 0
  }
  const full = data.length - (data.length % 64)
  for (let off = 0; off < full; off += 64) block(data, off)
  const tail = new Uint8Array(data.length % 64 < 56 ? 64 : 128)
  tail.set(data.subarray(full))
  tail[data.length - full] = 0x80
  const bits = data.length * 8
  const dv = new DataView(tail.buffer)
  dv.setUint32(tail.length - 8, Math.floor(bits / 4294967296))
  dv.setUint32(tail.length - 4, bits >>> 0)
  for (let off = 0; off < tail.length; off += 64) block(tail, off)
  const out = new Uint8Array(32)
  const odv = new DataView(out.buffer)
  for (let i = 0; i < 8; i++) odv.setInt32(i * 4, H[i])
  return toHex(out)
}

/** sha256 hex of idx-ordered lines `${senderName}\t${sentAt}\t${body}` joined by '\n' (kind excluded). */
export async function messagesDigest(msgs: { senderName: string; sentAt: string; body: string }[]): Promise<string> {
  const text = msgs.map((m) => `${m.senderName}\t${m.sentAt}\t${m.body}`).join('\n')
  return sha256Hex(encoder.encode(text))
}
